...
class matern:
    ...
    def K(self, X, Xstar):
        ...
        """
        r = l2norm_(X, Xstar)
        bessel = kv(self.v, np.sqrt(2 * self.v + eps) * r / self.l)
        f = 2 ** (1 - self.v) / gamma(self.v) * (np.sqrt(2 * self.v  + eps) * r / self.l) ** self.v
        res = f * bessel
        res[np.isnan(res)] = 1
...
class matern32:
    ...
    def K(self, X, Xstar):
        ...
        """
        r = l2norm_(X, Xstar)
        one = (1 + np.sqrt(3 * (r / self.lv + eps) ** 2))
        two = np.exp(- np.sqrt(3 * (r / self.l  + eps) ** 2))
    ...
    def gradK(self, X, Xstar, param):
        ...
        if param == 'l':
            ...
            r = l2norm_(X, Xstar)
            one = (1 + np.sqrt(3 * (r / self.l  + eps) ** 2))
            two = np.exp(- np.sqrt(3 * (r / self.l  + eps) ** 2))
            return one * two
        elif param == 'sigman':
...
class matern52:
    ...
    def K(self, X, Xstar):
        ...
        one = (1 + np.sqrt(5 * r ** 2  + eps) + 5 * r ** 2 / 3)
        two = np.exp(-np.sqrt(5 * r ** 2  + eps))
        return self.sigmaf * one * two + self.sigman * kronDelta(X, Xstar)
    ...
    def gradK(self, X, Xstar, param):
        ...
        if param == 'l':
            ...
            one = (1 + np.sqrt(5 * (r / self.l) ** 2  + eps) + 5 * (r / self.l) ** 2 / 3)
            two = np.exp(-np.sqrt(5 * r ** 2  + eps))
            return one * two
        elif param == 'sigman':
            return kronDelta(X, Xstar)
